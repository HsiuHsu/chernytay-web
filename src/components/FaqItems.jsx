...
export const FaqItem1 = ({ classes }) => {
    const faqItem1Infos = [
        '冷氣機在經過一長時間運轉後 , 裡面的重要零件會因為冷熱交替的關係 , 附著難以清除的污垢 , 這時候如果沒有作好妥善的保養維修工作 , 除了會阻礙冷氣機的正常運轉外 , 還有可能加速零件的磨損 . 耗費電力 , 以致縮短冷氣機的使用壽命 !',
        '建議您冷房專用的冷氣機 , 二年需做一次專業保養。如果是冷暖兩用型的冷氣機一年做一次的專業保養。'
    ]
    return (
        <>{
            faqItem1Infos.map((item, index) => (
                <Typography variant='body1' key={index} className={`${index !== faqItem1Infos.length - 1 && classes.mb6} ${classes.txt}`}>{item}</Typography>
            ))
        }</>
    )
}
export const FaqItem2 = ({ classes }) => {
    return (
...
export const FaqItem3 = ({ classes }) => {
    ...
    return (
        <List disablePadding className={classes.listGroup}>
            {
                faqItem3Infos.map(item => (<Typography key={item} component='li' variant='body1'>{item}</Typography>))
            }
        </List>
    )
}
export const FaqItem4 = ({ classes }) => {
    const faqItem4Infos = [
        ...
        <>
            <Paper sx={{ padding: '12px 16px', textAlign: 'center', marginBottom: 2 }}>
                <List disablePadding className={classes.listGroup}>
                    {
                        ['(每度用電定價，請參照台電電價表計時。)',
                            '每度用電單價 * (冷氣機每小時消耗電力 * 每日使用時間 * 冷氣機運轉率，通常為 (0.65) * 30天 / 1000 = 每月電費金額。',
                            '(舉例 : 窗型冷氣2500Kcal/hr (消耗電力1180w)。每天使用10小時，則電費每月約594元。)'
                        ].map((item, index) => (
                            <Typography key={index} variant={index == 1 ? 'body2' : 'body1'} component='li' sx={{ listStyle: 'none' }}>{item}</Typography>
                        ))
                    }
                </List>
            </Paper>
            <List disablePadding className={classes.listGroup}>
                {
                    faqItem4Infos.map(item => (<Typography key={item} component='li' variant='body1'>{item}</Typography>))
                }
            </List>
        </>
    )
}
export const FaqItem5 = ({ classes }) => {
    const faqItem5Infos = [
    ...
    return (
        <List disablePadding className={classes.listGroup}>
            {
                faqItem5Infos.map(item => (<Typography key={item} component='li' variant='body1'>{item}</Typography>))
            }
        </List>
    )
}
export const FaqItem6 = ({ classes }) => {
    const faqItem6Infos = [
        {
        ...
        <>
            {faqItem6Infos.map((item, index) => (
                <List disablePadding className={classes.listGroup} key={index}>
                    <Typography component='li' variant='body1' sx={{ listStyle: 'disc', listStylePosition: 'inside' }}>{item.title}</Typography>
                    {
                        item.details && item.details.map(detail => (<Typography key={detail} component='li' variant='body1' sx={{ listStyle: 'none', listStylePosition: 'inside' }}>{detail}</Typography>))
                    }
                </List>
            ))
            }
...
export const FaqItem7 = ({ classes }) => {
    const faqItem7Infos = [
        { p: '假設冷氣耗電功率是（1.2噸）812W，可是實際的效耗大約是在60%，因為設定的溫度25度過了，它的壓縮機就沒運轉了只有送風機在運轉。所以它的效耗電能大約812W的60%=490W。（調升一度約可以省6%）' },
        {
            title: '一天開8小時電費會多少？',
            p: '冷氣耗電量490w*8小時=3920w/h=3.92度每日用電量*台電每度大約在3元=11.8元'
        },
        {
            title: '一小時冷氣電費多少？',
            p: '11.8元/8小時=1.5元'
    ...
    return (
        <List disablePadding className={classes.listGroup} >
            {
                faqItem7Infos.map((item, index) => (
                    <Box key={index}>
                        {
                            item.title && <Typography component='li' variant='body1' sx={{ listStyle: 'disc', listStylePosition: 'inside' }}>{item.title}</Typography>
                        }
                        <Typography component='li' variant='body1' sx={{ listStyle: 'none' }}>{item.p}</Typography>
                    </Box>
                ))
            }
        </List>
...
export const FaqItem9 = ({ classes }) => {
    ...
    return (
        <List disablePadding className={classes.listGroup}>
            {
                faqItem9Infos.map(item => (<Typography key={item} component='li' variant='body1'>{item}</Typography>))
            }
        </List>
    )
}